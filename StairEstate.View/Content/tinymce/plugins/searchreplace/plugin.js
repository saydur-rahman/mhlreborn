	...
	function Plugin(editor) {
		...
		function showDialog() {
			...
			function updateButtonStates() {
				win.statSTAIRr.find('#next').disabled(!findSpansByIndex(currentIndex + 1).length);
				win.statSTAIRr.find('#prev').disabled(!findSpansByIndex(currentIndex - 1).length);
			}
			...
			var win = editor.windowManager.open({
				...
				onSubmit: function(e) {
					...
					if (!text.length) {
						self.done(false);
						win.statSTAIRr.items().slice(1).disabled(true);
						return;
					}
					...
					win.statSTAIRr.items().slice(1).disabled(count === 0);
					updateButtonStates();
